...
// Kullanım: node midi-to-arduino.js music.mid [tempo] [track] [melody-mode]
const midiFile = process.argv[2] || 'music.mid';
const customTempo = process.argv[3] || null;
const trackNumber = parseInt(process.argv[4]) || 0;
const melodyMode = process.argv[5] || 'smart'; // smart, highest, loudest, all

if (!fs.existsSync(midiFile)) {
  console.log('❌ MIDI dosyası bulunamadı:', midiFile);
  console.log('📝 Kullanım: node midi-to-arduino.js music.mid [tempo] [track] [melody-mode]');
  console.log('🎵 Melody modes: smart, highest, loudest, all');
  process.exit(1);
}
...
try {
  ...
  // Notaları çıkart - SADECE MELODY (tek nota)
  let melodyNotes = [];
  let currentTime = 0;
  let activeNotes = new Map(); // Note on/off takibi
  let noteEvents = []; // Tüm note eventlerini topla
  
  // Önce tüm note eventlerini topla
  ...
  // Melody çıkarma - aynı anda çalan notalardan en uygun olanı seç
  ...
  noteEvents.forEach(event => {
    if (event.type === 'on') {
      ...
      // Aynı anda çalan notalar varsa en uygun olanı seç
      ...
      // Süreyi Arduino formatına çevir
  ...
  console.log(`🎵 ${melodyNotes.length} melody nota çıkarıldı (toplam ${noteEvents.length / 2} MIDI notasından)`);
  console.log(`🎯 Filtreleme oranı: %${Math.round((1 - melodyNotes.length / (noteEvents.length / 2)) * 100)}`);
...
console.log('node midi-to-arduino.js track.mid 90 1');
console.log('node midi-to-arduino.js song.mid 120 0 highest  // en yüksek notalar');
console.log('node midi-to-arduino.js song.mid 120 0 loudest  // en güçlü notalar');
console.log('node midi-to-arduino.js song.mid 120 0 all      // tüm notalar');
console.log('');
